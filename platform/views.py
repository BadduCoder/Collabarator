...
from char_operators import StringProps
from models import User, Document, DocumentChange

def _doc_get_or_create(doc_id):
	try:
...
def index(request, document_id=None):
	...
	try:
		doc = Document.objects.get(doc_id=document_id)
	except Document.DoesNotExist:
		doc = Document(doc_id=document_id)
	...
	resp = render(request, 'platform/index.html', context)
		...
		with transaction.atomic():
			...
			try:
				...
				for c in changes_since:
					op2 = StringProps(json.loads(c.data))
					try:
						op, _ = StringProps.char_trans(op, op2)
					except:
